...
function submitBasicInfo(){
    //  realName   contactNumber   email  email_validate_code   modify_basic_info_btn
    $("#modify_basic_info_btn").click(function(){
        var status = $("#basic_info_model_form").valid();
        if(status == false) return;
        $.ajax({
            type: "POST",
            url: modifyAdminInfoUrl,
    ...
    $("#modify_passwd_btn").click(function(){
        ...
        $.ajax({
            ...
            success: function(data){
                ...
                var msg = data.msg;
                if(status == "200")  //学生注册成功
                    model_tip_show('model_tip','model_tip_content','密码成功',refreshToAdminPersonInfoPage);
                else if(status == "500")  //服务器原因失败
                    model_tip_show('model_tip','model_tip_content','服务器繁忙，请稍后再试');
                else
                    model_tip_show('model_tip','model_tip_content','服务器繁忙，请稍后再试');
